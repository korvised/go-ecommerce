...
func UpdateProductBuilder(
	db *sqlx.DB,
	req *products.Product,
	filesUsecases filesUsecases.IFilesUsecase,
) IUpdateProductBuilder {
...
func (b *updateProductBuilder) updateCategory() error {
	...
	if _, err := b.tx.ExecContext(
		...
		b.tx.Rollback()
		return fmt.Errorf("update product category failed: %v", err)
	}
	...
	if len(images) > 0 {
		...
		for _, img := range images {
			deleteFileReq = append(deleteFileReq, &files.DeleteFileReq{
				Destination: fmt.Sprintf("product/%s", img.FileName),
			})
		}

		fmt.Println(deleteFileReq)

		if err := b.filesUsecases.DeleteFileOnStorage(deleteFileReq); err != nil {
			log.Printf("delete image failed: %v\n", err)
		}
	}
...
func (en *updateProductEngineer) SumQueryField() {
	...
	for i, field := range fields {
		...
		if i != len(fields)-1 {
			en.builder.setQuery(query + field + ",")
		} else {
			en.builder.setQuery(query + field)
		}
	}
